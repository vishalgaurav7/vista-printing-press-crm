...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $myuser = trim($_POST['myuser']); // Renamed from username
    $email = !empty($_POST['email']) ? trim($_POST['email']) : null; // Email is optional
    ...
    try {
        ...
        $stmt->bindParam(':myuser', $myuser);
        $stmt->execute();

        if ($stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new Exception("The username '{$myuser}' already exists. Please choose a different one.");
        }

        // Check if the email already exists (only if provided)
        if ($email) {
            ...
            if ($stmt->fetch(PDO::FETCH_ASSOC)) {
                throw new Exception("The email '{$email}' is already registered. Please use a different email.");
            }
        }
        ...
        $sql = "INSERT INTO users (username, email, password, role, department_id) 
                VALUES (:myuser, :email, :password, :role, :department_id)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':myuser', $myuser);
        $stmt->bindParam(':email', $email);
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['successMessage'] = "User '{$myuser}' created successfully!";
...
<div class="task-cnt">
    <div class = "form-group w30">
        ...
        <input type="text" name="myuser" id="myuser" required>
    </div>
<br/>
